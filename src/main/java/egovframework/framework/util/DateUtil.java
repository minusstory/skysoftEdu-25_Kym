...
//	}
     ...
     * @param year 년도
     * @param month 월
     * @return ArrayList 달력리스트
     * 	 	[{"day=0"},{"day=0"},{"day=0"},{"day=1"},{"day=2"},{"day=3"},{"day=4"}]1주
	 * 		[..]2주
	 * 		[...]3주
	 * 		[{"day=30"},{"day=31"},{"day=0"},{"day=0"},{"day=0"},{"day=0"},{"day=0"}]4주
     */
    public static ArrayList getDayCalendar(int year, int month)
	throws Exception
	{
		...
		for(int i = 0; i <= getMonthWeek(year, month, lastDay); i++)
		{
			ArrayList weekList = new ArrayList();

			for(int k = 0; k < 7; k++)
			{
				if(lastDay <= day)break;
				HashMap weekMap = new HashMap();
				day++;

				if( day == 1 )
				{
					int defaultDay = getWeek(year, month, day);

					if( defaultDay > 1)
					{
						for(int d = 1; d < defaultDay; d++)
						{
							HashMap nullMap = new HashMap();
							nullMap.put("day", Integer.toString(0));
							weekList.add(nullMap);
				...
				weekMap.put("day", Integer.toString(day));
				weekList.add(weekMap);

				if(day == lastDay)
				{
					int endDay = getWeek(year, month, day);
					for(int d = endDay; d < 7; d++){
						HashMap endMap = new HashMap();
						endMap.put("day", Integer.toString(0));
						weekList.add(endMap);
					}
				}
			}
			jsonArray.add(weekList);
		}

		//공백 리스트 삭제
		for(int i = 0; i < jsonArray.size(); i++) if(jsonArray.get(i).equals(new ArrayList()))jsonArray.remove(i);
    ...
    throws Exception
    {
		int result = DateUtil.getWeek(year, month, day);

		if(result == 1)return "일";
		else if(result == 2)return "월";
		else if(result == 3)return "화";
		else if(result == 4)return "수";
		else if(result == 5)return "목";
		else if(result == 6)return "금";
		else if(result == 7)return "토";
		else return "";
    }
    ...
    /**
     * 오늘 일자를 가져옵니다.
     * @param format 날짜형식(yyyyMMdd, yyyy-MM-dd)
     * @return String 날짜
     *
     */
    ...
    /**
     * 날짜 계산을 진행합니다.
	 * @param divisionDate 기준일 날짜 yyyyMMdd
	 * @param day +,-1일자 dd
	 * @param outputFormat 반활할 출력 포맷 ex)yyyyMMdd
     * @return String 일수
     *
     */
	public static String getPrevDate(String divisionDate, int chk, String outputFormat)
	throws Exception
	{
		Calendar cal = Calendar.getInstance();

		int year = Integer.parseInt(divisionDate.substring(0,4));
		int month = Integer.parseInt(divisionDate.substring(4,6));
		int day = Integer.parseInt(divisionDate.substring(6,8));

		cal.set(year, month - 1, day);

		cal.add(cal.DATE, chk);
	...
	public static String getPrevMonth(String divisionDate, int day, String outputFormat)
	throws Exception
	{
		Calendar cal = Calendar.getInstance();
    ...
    /**
     * 주민등록 번호를 가지고 나이를 가져온다
     * @param residentId 주민등록 번호
     * @return int 현재년도에서 주민번호 년도를 뺀 나이
     *
     */
    public static int getAgeFromResidentId1(String residentId) {
        int birthYear = 0;
        try {
            if (residentId.length() != 13)
                return 0;
            birthYear = Integer.parseInt(residentId.substring(0, 2));
            if (birthYear < 20)
                birthYear += 2000;
            else
                birthYear += 1900;

            int thisYear = Integer.parseInt(getToday().substring(0,4));
            //String today = getToday();
            //String birthday = getToday().substring(0,4) + residentId.substring(2, 6);
    ...
    /**
     * 주민등록 번호를 가지고 만나이를 가져온다
     * @param residentId 주민등록 번호
     * @return int 현재년도에서 주민번호 년도를 뺀 만나이(생일 비교)
     *
     */
    public static int getAgeFromResidentId2(String residentId) {
        int birthYear = 0;
        int age = 0;
        int birthDay = 0;
        ...
        try {
            ...
            birthYear = Integer.parseInt(residentId.substring(0, 2));
            birthDay = Integer.parseInt(getToday().substring(0,4) + residentId.substring(2, 6));
            int thisYear = Integer.parseInt(getToday().substring(0,4));
    ...
    /**
     * 생년월일(yyyymmdd)를 가지고 만나이를 가져온다
     * @param 생년 월일 (yyyymmdd)
     * @return int 현재년도에서 생년(yyyy)을 뺀 만나이(생일 비교)
     *
     */
    public static int getAgeFromBirthdate(String birthdate) {
        ...
        try {
            ...
            birthDay = Integer.parseInt(getToday().substring(0,4) + birthdate.substring(4, 8));
            int thisYear = Integer.parseInt(getToday().substring(0,4));
